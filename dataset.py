...
def parse_cpi_stream(f, comment=b'#'):
    ...
    for line in f:
        ...
        try:
            ...
            label = -999
        if '.' in compound.decode(): continue # ignore compounds with '.' in them
        if compound is not None:
            yield compound, sequence, label

def seq_cat(hp, prot):
    x = np.zeros(hp['cnn_prot_length'])
    for i, ch in enumerate(prot[:hp['cnn_prot_length']]): 
        x[i] = prot[i]
    return x  
...
def load_data(datafile, hp, encoder, molgraph_dict, device, sanity_check=False):
    with open(datafile, 'rb') as f:
        ...
        for compound, sequence, label in parse_cpi_stream(f):
            ...
            if atoms is None:
                print("failure in compound  {}".format(compound.decode()))
                continue
            ...
            if hp['prot_model'] == 'cnn': # make all sequences of same length for cnn
                sequence = seq_cat(hp, sequence)
            sequences.append(sequence)
            labels.append(np.array([float(label)]))
    
    sequences = [torch.from_numpy(sequence).long().to(device) for sequence in sequences]
    labels = torch.from_numpy(np.stack(labels)).float().to(device)
    return compounds, sequences, labels, molgraph_dict

def split_train_val(df, trans, max_mz, ratio = 0.9):
    all_ik = pd.unique(df.InChIKey)
    ik_msk = np.random.random(all_ik.shape)
    train_ik = [ik for ik, msk in zip(all_ik, ik_msk) if msk < ratio]
    test_ik = [ik for ik, msk in zip(all_ik, ik_msk) if msk >= ratio]
    return df2list(df[df.InChIKey.isin(train_ik)].reset_index(drop=True), trans, max_mz), df2list(df[df.InChIKey.isin(test_ik)].reset_index(drop=True), trans, max_mz)

class CPIDataset(Dataset):
    def __init__(self, compounds, sequences, labels, hp, encoder, mol_dict=None):
...
def create_train_val_dataset(mode, hp, encoder, device, molgraph_dict, output):
    ...
    compounds, seq, labels, molgraph_dict = load_data(hp['train_data'], hp, 
                                                      encoder, molgraph_dict, device, hp['sanity_check'])

    if mode == 'both': #this means both train and valid data set files are supplied
        train_comp, train_seq, train_labels = compounds, seq, labels
        valid_comp, valid_seq, valid_labels, molgraph_dict = load_data(hp['valid_data'], hp, 
                                                                       encoder, molgraph_dict, device, hp['sanity_check'])
    ...
    train_ds = CPIDataset(train_comp, train_seq, train_labels, hp, encoder, molgraph_dict)
    val_ds = CPIDataset(valid_comp, valid_seq, valid_labels, hp, encoder, molgraph_dict)
...
def create_test_dataset(hp, encoder, device, molgraph_dict, output):
    ...
    test_compounds, test_seq, test_labels, molgraph_dict = load_data(hp['test_data'], hp, 
                                                      encoder, molgraph_dict, device, hp['sanity_check'])
        
    Print("Test Set with {} examples".format(len(test_seq)), output)
    test_ds = CPIDataset(test_compounds, test_seq, test_labels, hp, encoder, molgraph_dict)
